...
if ($dwn_ng_assign)
{
  if (!$mgmt_server || !$node_input_list)
  {
    print "Option needs parameters --mgmt_server|m <input_file> and --node_input_file <input_file>\n\n";
    exit 0;
  }
  ...
  if (!-f "/etc/opt/OV/share/conf/OpC/mgmt_sv/reports/C/Nodegroup-Overview_migtool.sql")
  {
    print "\nSQL script /etc/opt/OV/share/conf/OpC/mgmt_sv/reports/C/Nodegroup-Overview_migtool.sql NOT FOUND!";
    exit 0;
  }
...
##########################################################
# Sub that downloads the assignment nodegroups for managed nodes
#	@Parms:
#			$input_file:		input file in csv format (ip;node_fqdn;machine_type)
#     $mgm_server_file:     input file with hpom servers
#     $ng_dwn_file_path_location:      path in which will be downloaded nodegroup assignment
#     $date_time:     hpom date and time
#	Return:
#			$ng_assign_file_name_out : name of file generated.
###########################################################
sub download_ng_assignment
{
  my ($input_file, $mgm_server_file, $ng_dwn_file_path_location, $date_time) = @_;
  ...
  while(<INPUT_NODE_FILE>)
  {
    ...
    {
      chomp($mgmt_server = $_);
      if($node_name =~ m/$mgmt_server/)
      {
        print "Matched: $node_name =~ $mgmt_server\n";
        next;
      }
    }
